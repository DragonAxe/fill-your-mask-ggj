...
func process(entities: Array[Entity], components: Array, _delta: float):
    for e_player in entities:
        ...
        if c_swag_item.type != C_SwagItem.SwagType.NONE:
            ...
            e_player.remove_relationship(Relationship.new(C_Holding))
            print("Relationship? ", e_player.has_relationship(Relationship.new(C_Holding)))
            return
        ...
        e_player.add_relationship(Relationship.new(C_Holding, e_pickup))
